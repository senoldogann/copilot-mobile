...
export type SessionBehaviorPreferences = {
    agentMode: AgentMode;
...
export async function syncSessionPreferences(
    ...
    const c = getClient();
    const sessionStore = getBridgeSessionState();
    const behaviorPreferences = preferences ?? {
    ...
    try {
        await c.sendMessage("session.mode.update", {
            sessionId,
            agentMode: behaviorPreferences.agentMode,
        });
        await c.sendMessage("permission.level.update", {
            sessionId,
            permissionLevel: behaviorPreferences.permissionLevel,
        });
    } catch (error) {
        setBridgeConnectionError(
